        ...
        while True:
            ...
            try:
                ...
                if get_running_task:
                    ...
                    for i in get_running_task:
                        # if i.last_heartbeat:
                            self.stdout.write(f'Task running or fucked up for {i.status}: {i.url} Last heartbeat:= {i.last_heartbeat}')
                            i.status = i.STATUS_PENDING
                            i.save(update_fields=["status"])
                            # i.save()
                            self.stdout.write(f' Fewww... Task updated from RUNNING to {i.status} ')
