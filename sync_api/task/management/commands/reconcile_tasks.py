    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if get_running_task.exists():
                for task_ in get_running_task.iterator(chunk_size=100):
                    if task_.retry_count < Task.MAX_RETRIES:
 
                        rows = Task.objects.filter(
                            pk=task_.pk,
                            status=Task.STATUS_RUNNING,
                            version=task_.version
                        ).update(
                        ...
                        if rows == 0:
                            self.stdout.write(
                                f'Skipped {task_.url}: already changed by another process'
                            )
                            continue

                        self.stdout.write(
                            f'Task reset: {task_.url} → PENDING (retry {task_.retry_count + 1}/{Task.MAX_RETRIES})'
                        )
